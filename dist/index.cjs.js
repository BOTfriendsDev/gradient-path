...
var averageSegmentJoins = function averageSegmentJoins(
  ...
  for (var i = 0; i < outlinedData.length; i++) {
    ...
    currentSamples[currentMiddle - 1] = new Sample(
      _objectSpread2({}, combine(currentSamples, currentMiddle - 1, p0Average))
    );
    currentSamples[currentMiddle] = new Sample(
      _objectSpread2({}, combine(currentSamples, currentMiddle, p1Average))
    );
    nextSamples[0] = new Sample(
      _objectSpread2({}, combine(nextSamples, 0, p0Average))
    );
    nextSamples[nextEnd] = new Sample(
      _objectSpread2({}, combine(nextSamples, nextEnd, p1Average))
    );
  }
...
var GradientPath = /*#__PURE__*/ (function() {
  function GradientPath(_ref) {
    var path = _ref.path,
      ...
      _ref$keepOriginalPath = _ref.keepOriginalPath,
      keepOriginalPath =
        _ref$keepOriginalPath === void 0 ? false : _ref$keepOriginalPath;
    ...
    this.keepOriginalPath = keepOriginalPath; // Store the render cycles that the user creates
        ...
        if (type === 'path') {
          ...
          for (var _j = 0; _j < renderCycle.data.length; _j++) {
            var _renderCycle$data$_j = renderCycle.data[_j],
              ...
              _progress = _renderCycle$data$_j.progress; // Create a circle for each sample and append it to its elemGroup
